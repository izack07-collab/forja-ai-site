...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between gap-4 py-6 text-left text-base md:text-lg text-ink font-medium tracking-tightish transition-colors hover:text-accent-deep [&[data-state=open]>svg]:rotate-180",
        className,
      )}
      {...props}
    >
